...
comentaris = LOAD '/user/cloudera/pig_latin/critiquescinematografiques.csv' USING org.apache.pig.piggybank.storage.CSVExcelStorage(',', 'YES_MULTILINE')  AS (text:chararray, label:int, id:int);
pelis = LOAD '/user/cloudera/pig_latin/titol.csv' USING org.apache.pig.piggybank.storage.CSVExcelStorage(',', 'YES_MULTILINE')  AS (id:int, nom_pelicula:chararray);
...
dictionary = load '/user/cloudera/pig_latin/AFINN.txt' using PigStorage('\t') AS(word:chararray,rating:int);
...
STORE pelis_final INTO '/user/cloudera/WorkspacePigPractica/out_valorOpinio' USING org.apache.pig.piggybank.storage.CSVExcelStorage(',', 'YES_MULTILINE');
